...
# -----------------------------------------------------------------------------------------
# MambaVision 3D Implementation (Hybrid Mamba + Transformer)
# -----------------------------------------------------------------------------------------
...
class MambaVisionLayer3d(nn.Module):
    def __init__(self, dim, depth, num_heads, window_size, downsample=True,
        ...
        for i in range(depth):
            mixer_type = "attention" if (i in transformer_blocks) else "mamba"
            self.blocks.append(
                Block3d(dim, num_heads, mixer_type=mixer_type, mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, 
                        drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path)
            )
            
        # Downsample (usually at the beginning of next stage, simplified here to be part of layer or separate)
        # In MambaVision code, downsample is passed.
        if downsample:
            self.downsample = nn.Sequential(
                nn.Conv3d(dim, 2 * dim, kernel_size=3, stride=2, padding=1, bias=False),
                nn.BatchNorm3d(2 * dim) # Or LayerNorm/GroupNorm often better for Mamba
                # MambaVision uses Conv2d downsample. We use Conv3d.
            )
        else:
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            is_attn = isinstance(blk.mixer, Attention3d)
            if is_attn:
                # Reshape back to image
                # Assuming shape remains consistent (no downsample INSIDE blocks)
                x_Reshaped = x_flat.transpose(1, 2).view(B, C, D, H, W)
                ...
                x_flat = x_Reshaped.flatten(2).transpose(1, 2)
            else:
                x_flat = blk(x_flat)
...
class MambaVisionEncoder3d(nn.Module):
    def __init__(self, input_size, input_channels=1, dims=[64, 128, 256, 384], 
                 depths=[2, 2, 6, 2], window_sizes=[8, 8, 8, 8]):
        super().__init__()
        self.dims = dims
        self.output_channels = dims
        self.strides = [2, 2, 2, 2] # Stem / Downsamples
        self.kernel_sizes = [3] * len(dims)
        self.conv_pad_sizes = [[1]] * len(dims)
        
        # Stem
        self.stem = nn.Sequential(
            nn.Conv3d(input_channels, dims[0], 3, stride=2, padding=1),
        ...
        for i in range(len(dims)):
            # Stage blocks
            transformer_blocks = list(range(depths[i] // 2, depths[i])) if i >= 2 else []
            
            l = MambaVisionLayer3d(
                dim=dims[i],
                depth=depths[i],
                num_heads=dims[i] // 32 if dims[i] >= 32 else 1,
                window_size=window_sizes[i],
                downsample=False, # We handle downsample separately
                drop_path=dpr[idx:idx+depths[i]],
            ...
            if i < len(dims) - 1:
                ds = nn.Sequential(
                    nn.Conv3d(dims[i], dims[i+1], 3, stride=2, padding=1),
...
class MambaVisionUNet3d(nn.Module):
    def __init__(self, input_size, input_channels, num_classes, deep_supervision=True):
        super().__init__()
        self.encoder = MambaVisionEncoder3d(input_size, input_channels)
        n_stages = len(self.encoder.dims)
        n_conv_decoder = [1] * (n_stages - 1)
...
def create_mambavision_3d(input_size, input_channels=1, num_classes=2, **kwargs):
    """Create MambaVision 3D model (Hybrid Attention-Mamba Encoder)"""
    return MambaVisionUNet3d(input_size, input_channels, num_classes, **kwargs)
...
if __name__ == "__main__":
    ...
    print("\nTesting MambaVisionUNet3d...")
    model_vis = create_mambavision_3d(input_size=(64, 64, 64), input_channels=1, num_classes=3)
    out = model_vis(x)
    print(f"Input: {x.shape}, Output: {out.shape if not isinstance(out, list) else [o.shape for o in out]}")
